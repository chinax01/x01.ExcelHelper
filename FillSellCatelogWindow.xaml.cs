...
{
	/// <summary>
	/// 为系统销售销退添加分类信息，并生成各学校非免明细表
	/// </summary>
	public partial class FillSellCatelogWindow : Window
	{
		...
		void FillButton_Click(object sender, RoutedEventArgs e)
		{
			...
			var orgSheet1 = GetSheet(orgBook, OriginSheet1Name);	// 原始销售
			var orgSheet2 = GetSheet(orgBook, OriginSheet2Name);	// 原始销退
			var orgSheet3 = GetSheet(orgBook, OriginSheet3Name);	// 空缺
			var destSheet4 = GetSheet(orgBook, OriginSheet4Name);	// 系统销售
			var destSheet5 = GetSheet(orgBook, OriginSheet5Name);	// 系统销退
		              ...
		              bool isSell = true)
		{
			...
			for (int orgRow = orgStartRow - 1; orgRow < orgEndRow; orgRow++) {
				for (int orgCol = orgStartCol - 1; orgCol < orgEndCol; orgCol++) {
					if (org.GetRow(orgStartRow-1) == null) continue;
					...
					for (int dr = destStartRow-1; dr < destEndRow; dr++) {
						if (dest.GetRow(dr) == null) continue;
						...
						if (orgClientIdRowValue == destClientIdValue) {
							...
							for (int or = orgStartRow - 1; or < orgEndRow; or++) {
								if (org.GetRow(or) == null) continue;
								...
								if (destDate == orgDate) {
									...
									double orgTotal = orgTotalCell.CellType == CellType.Numeric 
										? orgTotalCell.NumericCellValue : double.MaxValue;
									if (orgTotal == double.MaxValue)
										continue;
									...
									double destTotal = destTotalCell.CellType == CellType.Numeric 
										? destTotalCell.NumericCellValue : double.MaxValue;
									if (destTotal == double.MaxValue)
										continue;
		...
		{
			if (dest == null) throw new Exception("dest must not be null");
			
			List<string> names = new List<string>();
			for (int or = org1StartRow - 1; or < org1EndRow; or++) {
				if (org1 == null) break;
				if (org1.GetRow(or) == null) continue;
			...
			foreach (var name in names) {
				...
				for (int r2 = org2StartRow - 1; r2 < org2EndRow; r2++) {
					if (org2 == null) break;
					var m = new CategoryModel();
					if (org2.GetRow(r2) == null) continue;
